...
const char * ssid = "Trail merah."; //WiFi SSID
...
void authentication(String content) {
  ...
  String url = "/projectsekolah/Authentication.php?card=" + String(cid);
  Serial.print("\nRequesting URL: ");
  Serial.println(url);
...
void sendData(String id) {
  ...
  address.replace(" ", "%20");
  String url = "/projectsekolah/record.php?id=" + id + "&w=" + Weight + "&address=" + address;
  Serial.print("\nRequesting URL: ");
  Serial.println(url);
  client.stop();
  // Use WiFiClient class to create TCP connections
